...
import { CATEGORIES, type Category, type CategoryTemplate, type MapFolder, type PropertyDef, type Pin, type World, type WorldMap } from './types'
...
function mapToRecord(map: WorldMap): { [key: string]: YamlValue } {
  ...
  if (map.folder) record.folder = map.folder
  return record
}

function folderToRecord(folder: MapFolder): { [key: string]: YamlValue } {
  const record: { [key: string]: YamlValue } = { id: folder.id, name: folder.name }
  if (folder.parentFolder) record.parentFolder = folder.parentFolder
  return record
}

function folderFromRecord(raw: YamlValue): MapFolder {
  if (!isRecord(raw)) throw new Error('Malformed map folder entry')
  const folder: MapFolder = { id: asString(raw.id), name: asString(raw.name) }
  if (typeof raw.parentFolder === 'string') folder.parentFolder = raw.parentFolder
  return folder
}
...
function mapFromRecord(raw: YamlValue): WorldMap {
  ...
  if (typeof raw.folder === 'string') map.folder = raw.folder
  return map
}
...
export function worldToMarkdown(world: World): string {
  ...
  data.mapFolders = world.mapFolders.map(folderToRecord)
...
export function worldFromMarkdown(md: string): World {
  ...
  const foldersRaw = Array.isArray(data.mapFolders) ? data.mapFolders : []
  ...
  return {
    ...
    mapFolders: foldersRaw.map(folderFromRecord),
