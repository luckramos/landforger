...
interface ImageInputProps {
  ...
  /** When false, the field only accepts uploaded files — no paste-a-link path.
      Maps use this: a chart is a static image the author uploads. */
  allowLink?: boolean
 ...
 */
export function ImageInput({ value, label, disabled = false, variant = 'square', size = 'medium', orientation = 'landscape', allowLink = true, onChange }: ImageInputProps) {
  ...
  // Upload-only fields skip the source menu and go straight to the file picker.
  const openSource = () => (allowLink ? openMenu() : pickFile())
  ...
  return (
    <div className={`${styles.imageInput} ${isBanner ? styles.imageInputBanner : ''}`} ref={anchorRef}>
      <button
        ...
        aria-label={label}
        aria-expanded={hasImage || !allowLink ? undefined : menuOpen}
        onClick={() => (hasImage ? setLightboxOpen(true) : openSource())}
      >
        {hasImage
          ? <img src={value} alt="" />
              ...
              <motion.div className={styles.lightboxInner} initial={false} exit={{ opacity: 0, scale: 0.98 }} transition={overlayExitTransition(motionScale)} onClick={(event) => event.stopPropagation()}>
                ...
                {!disabled && (
                  <div className={styles.lightboxActions}>
                    <button type="button" className={styles.tintButton} aria-label={`Replace ${label}`} onClick={() => { setLightboxOpen(false); openSource() }}><icons.upload size={14} /> Replace</button>
