  ...
  return (
    <Link to={`/w/${worldSlug}/p/${page.slug}`} className={styles.pageCard} aria-label={`${page.title}, ${page.summary}`}>
      ...
      <div className={styles.cardBody}>
        <span className={styles.eyebrow} style={{ color: `var(--cat-${page.category})` }}>{categoryMeta(page.category)?.icon} {categoryMeta(page.category)?.label}</span>
        <h2>{page.title}</h2>
        <p>{page.summary}</p>
        ...
        <div>
          {recent.map((page) => (
            <Link key={page.slug} to={`/w/${world.slug}/p/${page.slug}`}>
              <span className={styles.pageIcon} style={{ color: `var(--cat-${page.category})` }}>{categoryMeta(page.category)?.icon}</span>
              <strong>{page.title}</strong><span>{categoryMeta(page.category)?.label}</span>
  ...
  return (
    <main className={styles.list}>
      <header className={styles.listHeader}>
        <span className={styles.listIcon} style={{ color: meta ? `var(--cat-${meta.category})` : 'var(--bronze-light)' }}>{meta?.icon ?? '#'}</span>
